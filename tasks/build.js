...
function rollupOutputOpts(dest, opts) {
  const outputOpts = {
    file: `build/${dest}/${opts.filename || "rrulez.js"}`,
    format: opts.format,
    sourcemap: true,
  };
...
async function buildLibrary(dest, opts) {
  ...
  if (opts.minify && !TRUST_MINIFY) {
    const code = fs.readFileSync(`build/${dest}/rrulez.js`, "utf8"),
      ugly = UglifyJS.minify(code, {
        ...
        sourceMap: {
          filename: `build/${dest}/rrulez.js`,
        },
      });
    if (ugly.error) {
      console.error("Error uglifying", ugly.error);
    } else {
      fs.writeFileSync(`build/${dest}/rrulez.min.js`, ugly.code);
      fs.writeFileSync(`build/${dest}/rrulez.min.js.map`, ugly.map);
    }
  }
